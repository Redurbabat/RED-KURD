...
26 persistente Bereiche: **18 im `localStorage`** (alle in `KEYS`, `src/core/storage.ts:4–32`), einer im `sessionStorage`, einer in IndexedDB, zwei im Cache Storage, einer als Cookie und drei außerhalb des Browsers.
...
|---|---------|-----|---------|------------------|------------------------|-------|----------------|-----------------|------------|------------|
| 1 | Lernstand Sprache | Sprache | localStorage | `red-kurd-progress-v2` | `src/core/progress/progressStore.ts` | progressSelectors, courseRepository, shopStore, taskStore, achievementsStore, Wochenübersicht, ~20 Seiten | ja, `version: 2` | größter Speicher; `karten` bis ~2 384 Einträge (596 Wörter × 4 Fertigkeiten) ≈ 200–250 KB; `tage` hart auf 60 Einträge begrenzt (`progressStore.ts:219–221`) | ja, **einfach** (nur top-level `fortschritt`) | ja: v1→v2, dazu Kartenumbenennung |
| 2 | Lernprofil | Sprache | localStorage | `red-kurd-profile-v2` | `src/core/profile/profileStore.ts` | App, SideColumn, Onboarding, TodayPage, SessionPage, PracticeRunPage, ProgressPage, SettingsPage, Adventure-/Redlingo-Profil | ja, `version: 2` | < 300 B, konstant | ja, doppelt (`profil` + `speicher.profil`) | ja: v1→v2 |
| 3 | Oberfläche + Ansicht | Sprache | localStorage | `red-kurd-ui-v1` | `src/core/ui/uiStore.ts` | App, AppRouter, AnsichtSwitcher, appModeStorage, audioService, ProgressPage, SettingsPage | ja, `version: 2` — im **Wert**, während der Schlüsselname `v1` trägt | < 200 B, konstant | ja, doppelt (`ui` + `speicher.ui`) | ja: `red-kurd-modus` → UI; Wert-`version` < 2 → Theme `dark` |
| 4 | Laufende Sitzung | Sprache | localStorage | `red-kurd-session-v2` | `src/core/session/sessionStore.ts`, aufgerufen aus `ExercisePlayer.jsx:122` | SessionPage, TodayPage, RedlingoHomePage, ExercisePlayer | **nein** | 8–24 Übungen × ~250 B ≈ 2–6 KB; wird am Sitzungsende gelöscht | **nein — bewusst ausgeschlossen** (`NUR_LOKAL`, `storage.ts:169`) | ja: v1→v2 als 1:1-Kopie |
...
Vier Schlüssel aus Version 1 werden nur noch **gelesen** (`ALT` in `src/core/storage.ts:35–40`) und nach der Migration nie gelöscht:
...
Daneben kann `lies()` unter `<schlüssel>-defekt` einen Rohtext ablegen, wenn ein gespeicherter Wert sich nicht mehr als JSON lesen lässt (`storage.ts:69–78`). Diese Rettungskopien stehen nicht in `KEYS`, sind nicht im Export und werden nie aufgeräumt. Sie entstehen nur im Schadensfall.

---

## 3 · Feldstrukturen

Die folgenden Abschnitte beschreiben jeden Bereich so genau, dass sich daraus TypeScript-Typen ableiten lassen. Die Abschnitte 3.1 bis 3.15 beschreiben `localStorage`-Bereiche: Alle Werte liegen dort JSON-kodiert, gelesen und geschrieben über `lies()` / `schreibe()` in `src/core/storage.ts`. Danach folgen der `sessionStorage` (3.16), IndexedDB (3.17) und die Sicherungsdatei (3.18); die Bereiche außerhalb des Browsers stehen nur in der Übersicht in §2.

Reihenfolge und Nummern der Abschnitte folgen der Tabelle in §2.

### 3.1 `red-kurd-progress-v2` — Lernstand der Sprach-App

Quelle: `src/core/progress/progressStore.ts:9–27` (Konstante `LEER`). Dies ist der eine, geteilte Lernstand aller drei **Ansichten** der Sprach-App. Die Lernstände der drei anderen Apps liegen getrennt davon (§3.11).
...
| `tage` | `Record<string, Tag>` | Tagesaktivität, Schlüssel `"JJJJ-MM-TT"`, hart auf 60 Einträge begrenzt (`progressStore.ts:219–221`) |
...
**`Tag`** (`progressStore.ts:199`, Konstante `LEERER_TAG`):
...
**Achtung, ältere Tage sind ärmer.** Diese sechs Felder beschreibt `LEERER_TAG`; auf der Platte kann ein Tag deutlich weniger tragen. Die v1-Migration übernimmt `alt.tage` unverändert (`storage.ts`, Schritt 1). Wie wenig, ist nicht eindeutig belegt: `test/storage.test.js` und `test/storage-basis.test.js` halten für v1 eine **blanke Zahl** je Tag fest (`tage: { '2026-07-30': 20 }`), während der Kommentar an `LEER` (`progressStore.ts:20`) eine Drei-Feld-Form `{ aufgaben, richtig, sekunden }` zeigt. Beide dürften real existieren; sicher ist nur, dass ein migrierter v1-Tag **kein vollständiger `Tag`** ist. `zaehleAufgabe()` und `zaehleLernzeit()` sind unbedenklich, weil sie `LEERER_TAG` zuerst legen. `tagesStatistik()` (`progressSelectors.ts:175`) reicht den rohen Tag dagegen durch; deshalb schreibt `taskStore.ts:71` die Standardwerte noch einmal davor. Fällt diese Absicherung weg, wird aus `Math.min(tag.maxFolge, 3)` bei einem v1-Tag `NaN` und die Tagesaufgabe „3 richtige Antworten hintereinander" ist nie mehr abschließbar.

### 3.2 `red-kurd-profile-v2` — Lernprofil

Quelle: `src/core/profile/profileStore.ts:31–40` (Konstante `STANDARD`).
...
Quelle: `src/core/ui/uiStore.ts:6–14`. Hier liegt ausschließlich die Darstellung, kein Lernstand.

| Feld | Typ | Wirkung |
|------|-----|---------|
| `version` | `number` | `STANDARD` trägt `2` (`uiStore.ts:7`), `migriere()` legt den Schlüssel aber mit `version: 1` an (`storage.ts:300–308`). Werte < 2 lösen die einmalige Dark-Migration aus (`uiStore.ts:25`) — der Typ ist deshalb `number`, nicht das Literal `2` |
...
Geschrieben von `src/core/session/sessionStore.ts:6–9`, befüllt aus `src/features/exercise/ExercisePlayer.jsx:122`. **Ohne `version`-Feld** (§6). Steht seit der Aufnahme in `NUR_LOKAL` **nicht mehr in der Sicherungsdatei** (§3.18).
...
`sitzungLaden()` (`sessionStore.ts:15–26`) verwirft den Stand in vier Fällen — die Liste war bisher unvollständig:

1. `uebungen` fehlt, ist kein Array oder ist leer.
2. `index` ist keine ganze Zahl, negativ oder `>= uebungen.length`. Ein korrupter Zeiger würde sonst als Sitzung mit `NaN`-Prozent fortgesetzt.
3. `gespeichert` ist da, aber älter als 48 Stunden (`VERFALL_MS`, `sessionStore.ts:13`). Sonst verdrängt eine liegengebliebene Sitzung dauerhaft den Tagesplan auf der Heute-Seite.
4. `gespeichert` ist da, lässt sich aber nicht als Datum lesen — dann lieber verwerfen.

Ein Altbestand **ohne** `gespeichert` bleibt dagegen ausdrücklich nutzbar; `test/sessionStore.test.js` nagelt alle fünf Fälle fest. Das Feld ist damit wahlfrei, obwohl der Store es bei jedem Schreiben ergänzt.

### 3.5 `red-kurd-shop-v1` — Shop

Quelle: `src/core/shop/shopStore.ts:80`.

| Feld | Typ | Bedeutung |
|------|-----|-----------|
| `version` | `1` | Schemaversion |
| `gekauft` | `string[]` | Artikel-IDs; höchstens 6 |
| `aktiv` | `Record<string, string \| null>` | Artikelart → ausgerüstete Artikel-ID |

`aktiv` ist **kein** vollständiger `Record` über die vier Artikelarten: `STANDARD.aktiv` ist `{}` (`shopStore.ts:80`), ein Schlüssel entsteht erst beim ersten Ausrüsten. Der Wert kann `null` werden — `setzeAktiv()` legt einen Artikel wieder ab, indem es `null` schreibt, statt den Schlüssel zu entfernen (`shopStore.ts:149`). `aktiverArtikel()` fängt beides mit `id ? … : null` ab (`:155`). Mögliche Schlüssel sind `mascot`, `thema`, `klang` und `rahmen`; `verbrauch` steht hier nie, weil Verbrauchsartikel weder in `gekauft` noch in `aktiv` landen (`:137`).
...
Quelle: `src/core/achievements/achievementsStore.ts`.
...
Quelle: `src/core/tasks/taskStore.ts:15` (Konstante `STANDARD`).
...
- `tage` hat **keine Obergrenze**. Der Sprach-Lernstand kappt hart bei 60 Einträgen (`progressStore.ts:219–221`), hier wächst die Liste um einen Eintrag je aktivem Tag und wird nie aufgeräumt. Bei ~20 B je Eintrag ist das über Jahre unkritisch, aber es ist unbegrenzt.
...
Über `NUR_LOKAL` (`storage.ts:169`) **ausdrücklich aus Export und Import ausgenommen**. Grund: Das ist eine gerätelokale Entscheidung, kein Lernstand. Ein Import würde sonst die Anmeldeentscheidung eines fremden Geräts übernehmen.
...
Der Schlüssel steht in `TAB_KEYS` (`storage.ts:128–130`) und wird über `liesTab()` / `schreibeTab()` gelesen und geschrieben — er geht also **nicht mehr** am zentralen `storage.ts` vorbei. `TAB_KEYS` ist bewusst von `KEYS` getrennt: Der Wert überlebt den Tab nicht und gehört in keine Sicherung.
...
Erzeugt von `progressStore.exportiereAlles()` (`progressStore.ts:318–335`), aufgerufen aus `ProgressPage.jsx:422` und `SettingsPage.jsx:729`.
...
**Der Lernstand steht nur noch einmal in der Datei.** `exportiereAlles()` schneidet `fortschritt` aus dem `speicher`-Block heraus (`progressStore.ts:322`), weil die Kopie den größten Teil der Datei verdoppelte und vom Modul-Cache abweichen konnte. Ältere Sicherungsdateien *mit* der Kopie bleiben importierbar.
...
**`NUR_LOKAL` enthält jetzt zwei Bereiche** (`storage.ts:169`):
...
Daneben gibt es einen bewusst verlustbehafteten Zweitexport: `ankiZeilen()` (`progressStore.ts:337`) erzeugt `red-kurd-anki.txt` mit eindeutigen `de⇥ku`-Paaren aus den Kartenschlüsseln — ohne Stufe und Fälligkeit, ohne Reimport.

---

## 4 · Speicher, die am zentralen `storage.ts` vorbeigehen

`src/core/storage.ts:1–2` formuliert die Regel: „Alle Stores lesen und schreiben ausschliesslich ueber diese Datei."

Stand heute gilt sie **an genau einer Stelle nicht**: dem `audioService`, der über IndexedDB einen eigenen Persistenzkanal aufmacht. Ein `grep` nach `localStorage` und `sessionStorage` über `src/` findet außerhalb von `storage.ts` selbst keinen einzigen Zugriff mehr — die verbleibenden Treffer sind Kommentare und der Lektionstext in `codeLessons.js`, der `localStorage` erklärt.
...
**Was übrig bleibt:** Direkt darunter steht `entferne(ALT_PROFIL_SCHLUESSEL)` (`:746`) mit der Konstante `const ALT_PROFIL_SCHLUESSEL = 'red-kurd-profil-v1'` (`:32`). Der Zugriff läuft korrekt über `storage.ts`, aber der Name des Alt-Schlüssels steht doppelt: hier und in `ALT` in `storage.ts:37`. Der Grund für das Löschen ist gut und im Code begründet — bliebe das v1-Profil liegen, stellte `migriere()` es beim Neustart sofort wieder her und das Onboarding erschiene nie. Sauberer wäre, `ALT` aus `storage.ts` zu exportieren oder eine Funktion `loescheProfil()` in `profileStore` anzubieten, die Schlüssel, Alt-Schlüssel und Modul-Cache gemeinsam räumt.

### 4.2 Behoben: Sitzungsdauer im sessionStorage

Der Wert lag früher als `rk-dauer` unter einem eigenen Namensschema und wurde von zwei Seiten direkt über `window.sessionStorage` gelesen und geschrieben. Er heißt jetzt `red-kurd-tab-dauer`, steht in `TAB_KEYS` (`storage.ts:128–130`) und wird ausschließlich über `liesTab()` / `schreibeTab()` benutzt (`TodayPage.jsx:33`, `SessionPage.jsx:19`).

`TAB_KEYS` ist bewusst von `KEYS` getrennt und im Code begründet (`storage.ts:122–127`): tabgebundene Werte überleben den Tab nicht und wandern nie in eine Sicherung. Damit ist die frühere Kritik beantwortet — die Wahl `sessionStorage` ist jetzt eine dokumentierte Entscheidung an einer Stelle statt eine unerklärte Abweichung an zweien.
...
Diese Stellen laufen über `storage.ts`, brechen aber die Erwartung, dass Lesen nicht schreibt. Sie sind bekannt, kommentiert und derzeit gewollt — aber sie sind Fallen für den nächsten Umbau:
...
**3. Über `storage.ts`.** Schlüssel in `KEYS` eintragen, Zugriff ausschließlich über `lies()`, `schreibe()` und `entferne()`. Das gibt drei Dinge geschenkt: den Schutz vor privatem Modus und vollem Speicher (`schreibe()` gibt `false` zurück statt zu werfen), die Aufnahme in Export und Import, und die Cross-Tab-Synchronisation über den `red-kurd-`-Präfixfilter in `store.ts:42`. Wer daran vorbeigeht, verliert alle drei — der `audioService` in §4.3 belegt das. Für tabgebundene Werte gibt es `TAB_KEYS` mit `liesTab()` / `schreibeTab()`; sie stehen bewusst außerhalb von `KEYS`.

Den dritten Punkt muss ein neuer Store zusätzlich selbst abholen: `beiFremdaenderung()` aus `store.ts` abonnieren und dabei den eigenen Cache leeren. Sonst arbeitet ein zweiter Tab weiter mit alten Daten und überschreibt beim nächsten Speichern, was der erste geschrieben hat (siehe §6).
...
- Ist er eine gerätelokale oder flüchtige Größe (Anmeldung, laufende Sitzung, Fensterzustand), gehört er in `NUR_LOKAL` (`storage.ts:169`) — **mit Kommentar, warum**. Dort stehen heute `ohneKonto` und `sitzung`.
...
- Die Rettungskopien `<schlüssel>-defekt` (`storage.ts:69–78`) werden nie wieder gelesen, nie gelöscht und stehen in keinem Export. Sie sind für eine Handrettung gedacht, aber es gibt keinen Weg in der App, an sie heranzukommen.
